...
### Kwaito. A computational analysis of the South African (sub)genre {data-commentary-width=600}

![](/Users/Kalle/Desktop/Test/ro.jpeg){width=600px}
...
![](/Users/Kalle/Desktop/Test/btkwaito1.png){width=200px}


***
<div style="max-width:600px; margin: 0 auto;">
<h2>A brief history of Kwaito</h2>
...
I was curious whether Kwaito songs are produced live or, fully digitally and therefore synchronized to a fixed grid. 

The tempogram shows us a Kwaito song from the playlist. We are seeing a pretty steady line that doesn't change over time. This would suggest that the tempo remains stable over the course of the song. This would suggest Kwaito is digitally produced rather than recorded with live instruments. To be sure I did a tempogram for a few other tracks. The results were about the samen for the other cases. 

The tempo in bpm in this tempogram seems to be around 440. This is obviously very unlikely the real amount of beats per minute. Sometimes the tempo of a song is estimated in duplicate or multiple times. With downtempo songs like soul or slow Hip-Hop this happens some times. It shows why beat analysis could be problematic. A fix for this could be to set a range of bpm of which you are sure the track must be in between, for example between 50 and 200 beats per minutes. I have seen programs, for example DJ software, that actually are pretty good at bpm estimation. I guess this is one of the things that can be improved in the Spotify API.
...
***
![](/Users/Kalle/Desktop/Test/big_poppa.jpeg){width=100px} VS  ![](/Users/Kalle/Desktop/Test/Sandy.jpeg){width=100px}
...
laatste <-
  get_tidy_audio_analysis("4KGX6Nj07UkJlaI1pwkGu2") %>%
  select(segments) %>%
  unnest(segments) 
...
The Cepstrogram shows different timbre features for a particular segment. By zooming in and analyzing a smaller piece of audio we might get new insights. The song we're seeing is from the Kwaito playlist: 'I Love Music' by Lebo Mathosa. 

Looking at the cepstogram we see the features c02-c06 are most prevalent. There is a clear change in these dominant timbre features at the 15 seconds mark. Listening to the track you can tell it's the piano coming in. After this all of the prevelant features faint a little. Some of the features are more clearly defined then others. -c02 represents mostly low frequencies, -c03 mid frequencies, and -c04 represents the shorter and sharper sounding timbre features, called 'attack'. 

At the start of the song we hear a 4/4 percussion beat, which I would associate most with House Music. The bassdrum sounds deep and is pretty dominant. This explains prevalence of -c02/-c03 in the beginning. Attack speaks for the short punchy kick in the beginning I would guess. The percussion and bass are pretty dominant in the first half of the segment. At 0.15 sec the piano comes in. At this point the frequency spectrum has to be shared also with the piano frequencies. The lower- and mid frequencies therefore become less dominant in the second half of the segment. An explanation could be that lower frequencies were cut off by a filter to make room for the piano frequencies so the piano can really pop out from the start.
...
We took the opportunity to test and evaluate the possibilities and limitations of the Spotify API. First of all I want to say it is a very fun tool to use and it gives endless possibilities. While this is great, it could also be a pitfall. Sometimes you can get caught up in a single track-feature with the risk of getting in tunnel vision, making causal relationships that might not be there. If you want to compare whole genres of music it is therefore important to keep an overview. Sometimes taking a step back and thinking is this relevant, or asking yourself if your assumptions are correct can help to keep your head strait. In general, I would argue a strong base of statistical knowledge is very valuable in studying this kind of musical data.
